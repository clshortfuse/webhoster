...
export default class HttpHandler {
  ...
  async processMiddleware(transaction, middleware) {
    ...
    if (typeof middleware === 'object' && typeof middleware.onError === 'function') {
      ...
      try {
        ...
        console.warn('Error handler threw an error.', transaction.request.path, err);
    ...
    switch (typeof middleware) {
      ...
      case 'object':
        if (Array.isArray(middleware)) {
          ...
          for (let i = 0; i < len; i++) {
            const innerMiddleware = middleware[i];
  ...
  async handleHttp1Request(incomingMessage, serverResponse) {
    ...
    const { method, headers, socket, url: path } = incomingMessage;
        ...
        const promiseFn = async () => {
          try {
            const pushStream = await new Promise((resolve, reject) => {
              stream.pushStream(newHeaders, ((err, newStream) => (err ? reject(err) : resolve(newStream))));
            });
            pushStream.addListener('error', (err) => {
            ...
            console.error('onPushFailed', err, err.stack);
            throw err;
          }
        };
