...
export const DefaultMiddlewareSet = new Set();
...
export function handleRequest(request, response) {
  const iterator = DefaultMiddlewareSet.values();
  /**
   * @param {boolean} completed
   * @return {Promise<boolean>}
   */
  function middlewareLoop(completed) {
    if (completed) return Promise.resolve(completed);
    const result = iterator.next();
    if (result.done) return Promise.resolve(completed);
    return result.value(request, response).then(middlewareLoop);
  }
  return middlewareLoop(false).then(() => response).catch((err) => {
    console.error(err);
    throw err;
  });
}
...
export function handleHttpRequest(incomingMessage, serverResponse) {
  const request = new HttpRequest({
    ...
    method: incomingMessage.method?.toUpperCase(),
