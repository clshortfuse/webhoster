...
/** @type {Set<MiddlewareFunction[]>} */
export const MiddlewareChains = new Set();
MiddlewareChains.add(DefaultMiddlewareChain);
...
export function handleRequest(request, response) {
  const setIterator = MiddlewareChains.values();
  /**
   * @param {MiddlewareResultObject} [setResult]
   * @return {Promise<MiddlewareResultObject>}
   */
  function setLoop(setResult) {
    if (setResult?.completed) {
      return Promise.resolve(setResult);
    }
    const setIteration = setIterator.next();
    if (setIteration.done) return Promise.resolve(setResult);
    /** @type {MiddlewareFunction[]} */
    const chain = setIteration.value;
    const chainIterator = chain.values();
    /**
     * @param {MiddlewareResultObject} [chainResult]
     * @return {Promise<MiddlewareResultObject>}
     */
    function chainLoop(chainResult) {
      if (chainResult?.completed || chainResult?.break) {
        return Promise.resolve(chainResult);
      }
      const chainIteration = chainIterator.next();
      if (chainIteration.done) return Promise.resolve(chainResult);
      /** @type {MiddlewareFunction} */
      const middlewareFn = chainIteration.value;
      return Promise.resolve().then(() => middlewareFn(request, response)).then(chainLoop);
    }

    return chainLoop().then(setLoop);
  }

  return setLoop().then(() => response).catch((err) => {
    console.error(err);
    throw err;
  });
}
