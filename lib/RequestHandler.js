...
export function handleHttpRequest(incomingMessage, serverResponse) {
  ...
  responsePassthrough.on('data', function onData() {
    if (!serverResponse.headersSent) {
      console.warn('Writing without sending headers!');
    }
    responsePassthrough.off('data', onData);
  });
  responsePassthrough.pipe(serverResponse);

  const res = new HttpResponse({
    rawStream: responsePassthrough,
    onHeadersSent() {
...
export function handleHttp2Stream(stream, headers, responseOptions = {}) {
  ...
  responsePassthrough.pipe(stream);
