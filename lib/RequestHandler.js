  ...
  function handleMiddleware(middleware) {
    if (isInErrorState) {
      if (middleware && typeof middleware === 'object' && 'onError' in middleware && middleware.onError) {
        ...
        let returnValue;
        try {
          returnValue = middleware.onError({ req, res, err: caughtError });
        } catch (err) {
          isInErrorState = true;
          caughtError = err;
...
export function handleHttp2Stream(stream, headers, responseOptions = {}) {
  ...
  const pingerStream = new PassThrough({
    read(...args) {
      clearTimeout(pingTimeout);
      pingTimeout = setTimeout(() => stream.session?.ping(() => {}), STREAM_WAIT_MS);
      return PassThrough.prototype._read.call(this, ...args);
    },
  });
  stream.pipe(pingerStream);
  req.replaceStream(pingerStream);
