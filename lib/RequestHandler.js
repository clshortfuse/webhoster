...
export function handleRequest({ req, res }) {
  ...
  const previousWritable = res.replaceStream(newWritable);
  newWritable.on('data', function onData(chunk) {
    written = true;
    if (!res.headersSent) {
      if (hasEndCalled(newWritable)) {
        res.headers['Content-Length'] = chunk.length;
      } else {
        console.warn('RECEIVED PARTIAL CHUNK, NO CONTENT-LENGTH', chunk.length);
      }
      res.sendHeaders(false);
    }
    newWritable.off('data', onData);
  });
  newWritable.on('end', () => {
    if (!res.headersSent) {
      res.sendHeaders(false);
    ...
    if (!written) {
      if (res.status === 200) {
        console.warn('Sending status of 200 instead of 204!');
      }
    }
  });
  newWritable.pipe(previousWritable);
