# Advanced server sample (pattern-oriented)

This sample mirrors a real-world webhoster app layout without revealing any
closed-source details. It demonstrates:

- Nested routing with `PathMiddleware` and `MethodMiddleware`
- CORS setup for API routes
- JSON request parsing and JSON responses
- Streaming responses (SSE and binary)
- Chunked uploads with in-memory session assembly
- Static file serving
- Error handler pattern

> This sample is intentionally generic. Replace the service stubs with your
> own business logic.
